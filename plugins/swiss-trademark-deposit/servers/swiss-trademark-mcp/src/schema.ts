...
export function applySchemaMetadata(db: Database.Database): void {
...
export function assertCompatibleDatabase(db: Database.Database): void {
  ...
  if (missing.length > 0) throw new Error(`Incompatible Swiss trademark database: missing required tables ${missing.join(", ")}.`);
  ...
  if (schemaVersion === 0 && applicationId === 0) return;
  ...
  if (!tables.has("corpus_metadata")) throw new Error("Incompatible Swiss trademark database: corpus_metadata is missing.");
  const metadata = new Map(
    (db.prepare("SELECT key, value FROM corpus_metadata").all() as Array<{ key: string; value: string }>).map(

...
export class SwissTrademarkCorpus {
  ...
  manifest(): SourceManifest {
    ...
    return JSON.parse(readFileSync(this.manifestPath, "utf8")) as SourceManifest;
  }
  ...
  swissregSearchExamples(input: {
    ...
    if (ftsQuery) {
      rows = this.db
        .prepare(
          `SELECT g.id, g.urn, m.title, m.trademark_number, m.application_number, m.status, m.stage,
  ...
  getSwissregClassCombinations(input: { nice_classes: number[]; mode?: "superset" | "exact"; limit?: number }) {
    ...
    const rows = this.db
      .prepare(
        `SELECT g.urn, m.title, m.trademark_number, m.application_number, m.status, m.stage,
  ...
  swissregClassCombinations(input: { query?: string; classes_hint?: number[]; limit?: number } = {}) {
    ...
    const rows = this.db
      .prepare(
        `SELECT g.urn, g.class_number, g.goods_services, m.title
         FROM swissreg_goods_services g
         JOIN swissreg_marks m ON m.urn = g.urn
         WHERE 1=1${urnSql}
         ORDER BY g.urn, g.class_number`,
    ...
    const byUrn = new Map<string, { title: string; classes: Set<number>; terms: Map<number, string[]> }>();
    for (const row of rows) {
      const urn = String(row.urn);
      const entry = byUrn.get(urn) ?? { title: String(row.title ?? ""), classes: new Set<number>(), terms: new Map<number, string[]>() };
      const classNumber = Number(row.class_number);
      entry.classes.add(classNumber);
      const current = entry.terms.get(classNumber) ?? [];
      if (current.length < 3) current.push(truncate(row.goods_services, 120));
      entry.terms.set(classNumber, current);
      byUrn.set(urn, entry);
    }

    const combos = new Map<string, { classes: number[]; mark_count: number; example_titles: string[]; representative_terms: Record<string, string[]> }>();
    for (const entry of byUrn.values()) {
      const classes = [...entry.classes].sort((a, b) => a - b);
      if (classesHint?.length && !classesHint.every((classNumber) => entry.classes.has(classNumber))) continue;
      const key = classes.join(",");
      const combo = combos.get(key) ?? { classes, mark_count: 0, example_titles: [], representative_terms: {} };
      combo.mark_count += 1;
      if (combo.example_titles.length < 3 && entry.title) combo.example_titles.push(entry.title);
      for (const [classNumber, terms] of entry.terms) {
        const bucket = combo.representative_terms[String(classNumber)] ?? [];
        for (const term of terms) {
          if (bucket.length < 5) bucket.push(term);
        }
  ...
  tafGetDecision(input: { reference: string }) {
    ...
    const row = this.db
      ...
      .get(input.reference, `%${normalized}%`) as DbRow | undefined;
    if (!row) throw new Error(`TAF decision not found for reference ${input.reference}.`);
  ...
  filingIntakeCheck(input: {
    ...
    if (!input.goods_services?.length) missing_required.push("Goods/services list by Nice class.");

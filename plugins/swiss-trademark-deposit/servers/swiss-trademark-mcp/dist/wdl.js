...
await runCorpusServer("wdl", (server, corpus) => {
    server.registerTool("search_wdl", {
        title: "Search WDL",
    ...
    }, async (args) => toolResult(corpus.wdlSearchTerms({
        query: args.query,
        classes: args.class_number ? [args.class_number] : undefined,
        limit: args.limit,
        offset: args.offset,
    })));
    server.registerTool("get_wdl_terms_by_class", {
    ...
    server.registerTool("validate_term", {
    ...
    }, async (args) => toolResult(corpus.validateWdlTerm({
        term: args.terme,

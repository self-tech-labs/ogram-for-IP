#!/usr/bin/env node
import { z } from "zod";
import { goodsServicesTextListSchema, identifierSchema, limit50Schema, niceClassSchema, niceClassesSchema, offsetSchema, querySchema, shortTextListSchema, shortTextSchema, territoriesSchema, } from "./mcp-schemas.js";
...
await runCorpusServer("swissreg-corpus", (server, corpus) => {
    server.registerTool("search_swissreg_terms", {
    ...
    }, async (args) => toolResult(corpus.searchSwissregTerms({
        query: args.query,
    ...
    server.registerTool("get_class_combinations", {
    ...
    server.registerTool("get_sector_benchmark", {
    ...
    server.registerTool("list_mandataires", {
    ...
    server.registerTool("swissreg_search_examples", {
    ...
    server.registerTool("swissreg_class_combinations", {
    ...
    server.registerTool("swissreg_mark_detail", {
    ...
    server.registerTool("swissreg_mandataire_metadata", {
    ...
    server.registerTool("clearance_search_plan", {

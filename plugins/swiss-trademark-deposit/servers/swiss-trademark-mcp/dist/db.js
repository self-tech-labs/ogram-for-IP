...
function requireClasses(classes) {
    ...
    if (invalid !== undefined)
        throw new Error(`Invalid Nice class ${invalid}; expected an integer from 1 to 45.`);
    return [...new Set(normalized)].sort((a, b) => a - b);
}
function classWhere(classes, column = "class_number") {
    if (!classes?.length)
        return { sql: "", params: [] };
    return {
...
function sourceRef(row) {
    const parts = [];
    if (row.class_number)
        parts.push(`classe=${row.class_number}`);
    if (row.page)
        parts.push(`page=${row.page}`);
    if (row.line_page)
        parts.push(`ligne=${row.line_page}`);
...
function sourceAgeDays(sourceDate, now = new Date()) {
    if (!sourceDate)
        return null;
...
function isSwissDomicile(value) {
...
export class SwissTrademarkCorpus {
    ...
    tafExtractionStats() {
        try {
            const rows = this.db.prepare("SELECT extraction_quality_json FROM taf_entries").all();
            const qualities = rows.map((row) => parseJsonArray(row.extraction_quality_json, {})).filter((row) => row.parser);
    ...
    niceGetHeadings(input = {}) {
        const classes = requireClasses(input.classes);
        const where = classWhere(classes);
        const rows = this.db
            .prepare(`SELECT class_number, heading, source_file, source_date
    ...
    wdlSearchTerms(input = {}) {
        ...
        if (ftsQuery) {
            rows = this.db
                .prepare(`SELECT w.id AS term_id, w.class_number, w.term, w.admissible, w.comment, w.source,
           ...
           ORDER BY raw_score
           LIMIT ? OFFSET ?`)
                .all(ftsQuery, ...baseParams, limitPlusOne, offset);
            if (rows.length === 0) {
               ...
               ORDER BY raw_score
               LIMIT ? OFFSET ?`)
                        .all(anyQuery, ...baseParams, limitPlusOne, offset);
                }
            }
        }
        else {
           ...
           LIMIT ? OFFSET ?`)
                .all(...baseParams, limitPlusOne, offset);
        }
        if (ftsQuery && rows.length === 0) {
           ...
           ORDER BY w.class_number, w.term
           LIMIT ? OFFSET ?`)
                .all(`%${normalized}%`, ...baseParams, limitPlusOne, offset);
        }
    ...
    wdlTermsByClass(input) {
        const [classNumber] = requireClasses([input.class_number]) ?? [];
        const limit = Math.min(Math.max(Number(input.limit ?? 200), 1), 500);
        const offset = Math.max(Number(input.offset ?? 0), 0);
        const rows = this.db
            .prepare(`SELECT id AS term_id, class_number, term, admissible, comment, source, privileged, page, line_page
         FROM wdl_terms
         WHERE class_number = ?
         ORDER BY term
         LIMIT ? OFFSET ?`)
            .all(classNumber, limit + 1, offset);
        return {
            class_number: classNumber,
    ...
    swissregSearchExamples(input = {}) {
        ...
        if (input.mandataire_groups?.length) {
            throw new Error("mandataire_groups cannot be applied to Swissreg example rows in this corpus: the source only provides filter metadata, not row-level mandataire mapping. Use swissreg_mandataire_metadata for the available metadata.");
        }
        ...
        if (ftsQuery) {
           ...
           ORDER BY raw_score
           LIMIT ? OFFSET ?`)
                .all(ftsQuery, ...where.params, limitPlusOne, offset);
            if (rows.length === 0) {
                const anyQuery = toFtsAnyQuery(query);
                if (anyQuery && anyQuery !== ftsQuery) {
                    rows = this.db
               ...
               ORDER BY raw_score
               LIMIT ? OFFSET ?`)
                        .all(anyQuery, ...where.params, limitPlusOne, offset);
                }
            }
        }
        else {
           ...
           LIMIT ? OFFSET ?`)
                .all(...where.params, limitPlusOne, offset);
        }
        const hasMore = rows.length > limit;
        return {
            results: rows.slice(0, limit).map((row) => ({
    ...
    swissregMandataireMetadata(input = {}) {
        const { limit, offset } = pagination(input.limit, input.offset);
        const filters = [];
        const params = [];
        ...
        const rows = this.db
            .prepare(`SELECT mandataire_group, query, facet_value, result_count
        ...
        return {
            ...
            has_more: rows.length > limit,
            corpus_notice: "These are Swissreg filter metadata values only. They cannot be used as row-level filters for goods/services examples in the current corpus.",
        };
    }
    searchSwissregTerms(input = {}) {
        ...
        return {
            ...
            mandataire_filter_notice: "The source workbook does not provide row-level mandataire attribution in the Produits-services sheet, and Valeurs filtre facet IDs do not join to mark IDs. Results are not filtered by mandataire; use list_mandataires or swissreg_mandataire_metadata for available filter metadata.",
            requested_mandataire: input.mandataire,
        };
    }
    listSwissregMandataires() {
        const rows = this.db
            .prepare(`SELECT mandataire_group, COUNT(*) AS metadata_rows, SUM(COALESCE(result_count, 0)) AS total_result_count
         FROM swissreg_filter_values
        ...
        return {
            mandataires: rows,
            corpus_notice: "Mandataire groups are available as Swissreg filter metadata only. The current source does not map each goods/services row to a mandataire; Valeurs filtre facet IDs are not Swissreg mark IDs.",
        };
    }
    getSwissregClassCombinations(input) {
    ...
    getSwissregSectorBenchmark(input) {
        const [classNumber] = requireClasses([input.nice_class]) ?? [];
        const limit = Math.min(Math.max(Number(input.limit ?? 20), 1), 50);
        const rows = this.db
            .prepare(`SELECT urn, goods_services, language_hint
    ...
    swissregMatchingUrns(query, classesHint, limit = 1000) {
        const where = classWhere(classesHint, "g.class_number");
        const ftsQuery = toFtsQuery(query);
        if (!ftsQuery)
            return { urns: [], limited: false };
        let rows = this.db
            .prepare(`SELECT DISTINCT g.urn
         FROM swissreg_goods_fts
         JOIN swissreg_goods_services g ON g.rowid = swissreg_goods_fts.rowid
         WHERE swissreg_goods_fts MATCH ?${where.sql}
         LIMIT ?`)
            .all(ftsQuery, ...where.params, limit + 1);
        if (rows.length === 0) {
            const anyQuery = toFtsAnyQuery(query);
            if (anyQuery && anyQuery !== ftsQuery) {
                rows = this.db
    ...
    swissregMarkDetail(input) {
        const mark = this.db
            .prepare(`SELECT urn, internal_id, trademark_number, application_number, title, status, stage
         FROM swissreg_marks WHERE urn = ?`)
            .get(input.urn);
        if (!mark)
            throw new Error(`Swissreg mark not found for urn ${input.urn}.`);
        const classes = this.db
            .prepare(`SELECT class_number, goods_services, language_hint, source_date
        ...
        if (ftsQuery) {
           ...
           LIMIT ? OFFSET ?`)
            ...
            if (rows.length === 0) {
               ...
               ORDER BY raw_score
               LIMIT ? OFFSET ?`)
                        .all(anyQuery, ...params, limitPlusOne, offset);
                }
            }
        }
        else {
            rows = this.db
                .prepare(`SELECT e.*, 0 AS raw_score FROM taf_entries e
           WHERE 1=1${filterSql}
           ORDER BY e.page_start, e.entry_id
           LIMIT ? OFFSET ?`)
                .all(...params, limitPlusOne, offset);
        }
        const hasMore = rows.length > limit;
        return {
    ...
    tafGetEntry(input) {
        const row = this.db.prepare("SELECT * FROM taf_entries WHERE entry_id = ?").get(input.entry_id);
        if (!row)
            throw new Error(`TAF entry not found: ${input.entry_id}`);
    ...
    findSimilarTafSigns(input) {
        ...
        const ftsMatches = new Set(this.tafSearchPrecedents({ query: sign, classes, limit: 50 }).results.map((result) => String(result.entry_id)));
        const rows = this.db
            .prepare(`SELECT *, 0 AS raw_score
         FROM taf_entries
         WHERE 1=1
        ...
        const scored = rows
            ...
            const entryTags = parseJsonArray(row.risk_tags_json, []);
            const entryClasses = parseJsonArray(row.classes_json, []);
            ...
            const typeMatch = requestedType == null
                ? 0
            ...
            const score = Math.min(1, extractionScore.candidate_similarity * 0.3 +
    ...
    corpusStats() {
        ...
        const counts = {
            nice_headings: this.db.prepare("SELECT COUNT(*) AS count FROM nice_headings").get().count,
            wdl_terms: this.db.prepare("SELECT COUNT(*) AS count FROM wdl_terms").get().count,
            swissreg_goods_services: this.db.prepare("SELECT COUNT(*) AS count FROM swissreg_goods_services").get().count,
            swissreg_marks: this.db.prepare("SELECT COUNT(*) AS count FROM swissreg_marks").get().count,
            class_examples: this.db.prepare("SELECT COUNT(*) AS count FROM class_examples").get().count,
            taf_entries: this.db.prepare("SELECT COUNT(*) AS count FROM taf_entries").get().count,
        };
        return {
            ...
            freshness_notice: "Corpus data is local and date-stamped. Verify current IPI fees, filing requirements, and WIPO/Nice updates at the official sources at filing time.",
            official_links: OFFICIAL_LINKS,
        };
    }
    filingRequirementsSnapshot(input = {}) {
        return {
            official_sources_checked_at: OFFICIAL_SOURCE_CHECKED_AT,
            nice_classification_version: NICE_VERSION,
            ...
            relative_grounds_rule: "The IPI examines formal requirements and absolute grounds. It does not examine likelihood of confusion with earlier marks.",
    ...
    clearanceSearchPlan(input) {
        ...
        return {
            ...
            figurative_or_combined_mark_steps: input.mark_type && /figur|logo|combined|combin|image|3d|tridimension/i.test(input.mark_type)
                ? [
            ...
            limitation: "This is a search plan, not a complete clearance opinion. The local Swissreg corpus is only a drafting/example corpus and not a live register search.",
            official_links: OFFICIAL_LINKS,
        };
    }
    signRiskScreen(input) {
        const classes = requireClasses(input.classes);
        const sign = collapseForPlan(input.sign);
    ...
    filingIntakeCheck(input = {}) {
        ...
        if (!input.applicant?.name)
            missing_required.push("Applicant name and legal identity.");
        ...
        if (!input.sign?.text && !input.sign?.representation_provided)
        ...
        if (!input.territories?.length)
            next_questions.push("Is protection needed only in Switzerland or also abroad?");
        if (!input.risk_tolerance)
            next_questions.push("What opposition/refusal risk is acceptable for this client?");
        if (input.priority_claim?.claimed && !input.priority_claim.details)
    ...
    tafEntry(row) {
        return {
            ...
            articles: parseJsonArray(row.articles_json, []),
            classes: parseJsonArray(row.classes_json, []),
            taf_refs: parseJsonArray(row.taf_refs_json, []),
            risk_tags: parseJsonArray(row.risk_tags_json, []),
            ...
            extraction_quality: parseJsonArray(row.extraction_quality_json, {}),
        };
    }
    tafResult(row) {
        ...
        return {
            ...
            score: typeof row.raw_score === "number" ? Math.round(Math.abs(row.raw_score) * 1000) / 1000 : 0,
            extraction_confidence: typeof entry.extraction_quality?.confidence === "number" ? Math.round(entry.extraction_quality.confidence * 1000) / 1000 : null,

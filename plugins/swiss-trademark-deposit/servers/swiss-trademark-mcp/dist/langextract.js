...
export function cleanTafText(value) {
    return value
        .replace(/\[\[PAGE:\d+\]\]/g, "\n")
...
export function segmentTafReport(fullText) {
...
export function extractPageSpan(fullText, segment) {
...
export function extractTafTitle(text, index) {
...
export function extractTafReferences(text) {
    const refs = [
...
export function extractClassOutcomes(text) {
    const classLine = text.match(/Classes concern[ée]es\s*:\s*([^\n]+)/i)?.[1] ?? null;
    if (!classLine)
        return { classes: [], class_line: null };
    const results = [];
    ...
    if (results.length === 0) {
        for (const match of classLine.matchAll(/\b(\d{1,2})\b/g)) {
            const classNumber = Number(match[1]);
            if (classNumber >= 1 && classNumber <= 45)
                results.push({ class_number: classNumber, outcome: null });
        }
    }
    return { classes: results, class_line: collapseWhitespace(classLine) };
}
export function extractSignType(title) {
    const segment = title
        .split("/")
        .at(-1)
        ?.replace(/:$/, "")
...
export function extractRiskTags(text, title) {
    const normalized = normalizeText(`${title} ${text}`);
    const tags = new Set();
    const addIf = (tag, patterns) => {
...
export function extractTafSignCandidate(title) {
    const cleaned = collapseWhitespace(title)
        .replace(/^(acc|ref|part)\),?\s*/i, "")
        .replace(/^Liens:\s*/i, "");
    if (!cleaned || cleaned.startsWith("N°"))
        return null;
    const beforeNumber = cleaned.split(/\s+N°\s+/)[0]?.trim();
    if (beforeNumber && beforeNumber !== cleaned)
        return beforeNumber;
    const indicatorMatch = cleaned.match(/^(.+?)\s+(?:Est|N'est|Termes?|Courants?|Dans|A la|Le signe|La marque|Liens|Marque)\b/i);
    if (indicatorMatch?.[1])
        return collapseWhitespace(indicatorMatch[1]);
    if (cleaned.length <= 80 && !/Classes concern[ée]es/i.test(cleaned))
        return cleaned;
    return null;
}
export function classifyRequestedSignType(value) {
    const normalized = normalizeText(value ?? "");
    if (!normalized)
        return null;
    if (normalized.includes("combin"))
        return "combined";
    if (normalized.includes("figurat") || normalized.includes("logo") || normalized.includes("image"))
        return "figurative";
    if (normalized.includes("verb") || normalized.includes("word"))
        return "verbal";
    return null;
}
export function inferStructuralTags(sign, signType) {
    ...
    if (hasAny(["technique", "technical", "necessaire", "fonctionnel", "function"]))
        tags.add("technical_necessity");
    if (tokens.length >= 4 || /[.!?]/.test(sign))
        tags.add("slogan");
    const requestedType = classifyRequestedSignType(signType);
    if (requestedType === "figurative" || requestedType === "combined")
        tags.add("distinctiveness");
    return [...tags].sort();
}
function confidenceFor(entry, classLine) {
...
export function extractTafEntry(fullText, segment, index) {
...
export function summarizeTafExtractions(entries) {
...
export function scoreExtractedSign(input) {
    ...
    const lexicalOverlap = overlapRatio(contentWords(input.requested_sign), contentWords(`${input.candidate ?? ""} ${input.entry_title} ${truncate(input.entry_text, 900)}`));

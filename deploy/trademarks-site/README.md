...
The install button currently opens the published plugin (`asdk_app_6aa94b3c5bd081a4bbf7c8bf10af937a`). A new `0.3.0` plugin (`asdk_app_6ab4d705c86081a4ab5e7d5886adc6cd`) has been prepared with `https://trademarks.ogram.swiss/mcp` as its MCP URL. OpenAI requires a new plugin listing when the MCP server origin changes; an updated version of the existing plugin cannot switch from `workers.dev` to `trademarks.ogram.swiss`. After OpenAI approves and the new plugin is published, change the primary link in `public/index.html` to its public listing. `public/setup.js` reads that same link for the copied prompt.
